...
public class AdminQnaController {
	...
	public String shareDeleteController(@RequestBody List<BoardPostVO> deleteQnaBoard, Errors error, Model model) {

		log.info("deleteShareBoard : {}", deleteQnaBoard);
		log.info("error : {}", error);
		if (deleteQnaBoard.size() == 0) {
			model.addAttribute("msg", "empty");
		} else {
			int cnt = service.boardCheckedDelete(deleteQnaBoard);
			if (cnt > 0) {
				model.addAttribute("msg", "ok");
			} else {
				model.addAttribute("msg", "fail");
