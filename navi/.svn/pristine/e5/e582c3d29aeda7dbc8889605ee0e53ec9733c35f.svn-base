...
(function ($, undefined) {
        ...
        var core = {
            ...
            leftPanel: function (element) {
                ...
                var ganttLeftPanel = $('<div class="leftPanel"/>')
                    .append($('<div class="row header-gt gantt_column"/>')
                    .append($('<span class="th idx">순번</span><span class="th tite">제목</span><span class="th con">내용</span><span class="th nm">담당자</span>'))
                    .css("height", tools.getCellSize() * element.headerRows));

                var entries = []; 
             
                $.each(element.data, function (i, entry) {
                    // '<div class="row"'+i+'"flex">'
                    if (i >= element.pageNum * settings.itemsPerPage &&
                        (element.pageNum * settings.itemsPerPage + settings.itemsPerPage)) {
                        var dataId = ('id' in entry) ? '" data-id="' + entry.id : '';
                        //번호
                        entries.push(
                            '<div class="row idx td' + i +
                            (entry.idx ? '' : (dataId)) +
                            '" id="rowheader' + i +
                ...
                return ganttLeftPanel.append(entries);
            ...
            rightPanel: function (element, leftPanel /* <- never used? */) {
                ...
                // reused variables
                var $row = $('<div class="row header-gt"></div>');
                var i, len;
	...
	//여기여기 
                ...
                case "weeks":
                    ...
                    for (i = 0, len = range.length; i < len; i++) {
                        ...
                        dayArr.push(
                            ...
                            '" data-offset="' + i * tools.getCellSize() +
                            '" data-repdate="' + rday.getRepDate(settings.scale) + '">' +
                            '<div class="fn-label">' + week + '주</div></div>');
                    }
			...
			//여기에 데이터 꽂아야할 일감 데이터 꽂아서사용 ,class="taskLink'+i+'" 넣어둠
            createProgressBar: function (label, desc, classNames, dataObj) {
				i++;
                label = label || "";
				var i = 0;
                var bar = $('<div class="bar"><div class="fn-label"><a href="#" class="taskLink'+i+'">' + label + '</a></div></div>')
