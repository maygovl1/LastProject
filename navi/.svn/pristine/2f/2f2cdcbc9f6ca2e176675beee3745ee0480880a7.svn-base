   ...
   CKEDITOR.ClassicEditor.create(viewEdit, {
                ...
                removePlugins: [                  
            		...
            		name.each((i,v)=>{
            			console.log(i,v)
            			if($(v).attr("name")=="bpCn"){
            				let cn="<p>채팅 앱을 사용할 때 왜 꼭 채팅 비밀번호를 입력해야 하는 건가요?&nbsp;</p><p>이런 비밀번호가 필요한 이유가 있을까요?&nbsp;</p><p>저희가 사용하는 채팅 앱은 보안이 필요한 데이터를 다루지는 않는데, 왜 이렇게까지 보안에 신경을 써야 하나요?&nbsp;</p><p>그리고 이 비밀번호를 설정하는 것이 사용자 경험에 어떤 영향을 미칠까요? 사용자들은 이를 귀찮게 여길 수도 있지 않을까요? 이 모든 것들에 대한 설명이 궁금합니다.</p>"
            				$(v).val(cn)
